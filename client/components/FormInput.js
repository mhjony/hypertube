...
const FormInput = ({
  ...
  return (
    <>
      {label && (
        <label className="block uppercase text-gray-500 text-xs font-bold mb-2">{label}</label>
      )}
      ...
      <input
        ...
        onClick={onClick}
      />
      {errorMsg && <div className="mt-1 text-xs text-red-600 font-bold">{errorMsg}</div>}
