              ...
              {session && (
                <div className="flex justify-center items-center">
                  <div className="w-48 h-64 rounded-xl bg-gray-200 flex flex-col shadow">
                    <img className="w-auto rounded-t-xl" src={comment.user.avatar ? `http://localhost:8000/uploads/user/${comment.user.user_id}/${comment.user.avatar}` : 'http://localhost:8000/uploads/user/default/default.jpeg'/*`http://localhost:5000/uploads/user/${profile.user_id}/${profile.avatar}` */} alt="avatar" />
